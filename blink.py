    ...
    while cap.isOpened():
        ...
        if results.multi_face_landmarks:
            for face_landmarks in results.multi_face_landmarks:
                # Рисуем общие landmarks лица mp_drawing.draw_landmarks(
                image, face_landmarks, mp_face.FACEMESH_CONTOURS,
        ...
        #cv2.putText(image, f"Left EAR: {left_ear:.2f}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
        #cv2.putText(image, f"Right EAR: {right_ear:.2f}", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
        ...
        if avg_ear < sum(avg_open)/len(avg_open) - x and eye_open:
            blinks+=1
            cv2.putText(image, "BLINK DETECTED!", (10, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
            eye_open = False
